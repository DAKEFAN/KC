const GAME_CONFIG = {
    storageKey: 'museum-sim-save-v3',
    homeUnlockSavings: 100000,
    homeDownPayment: 100000,
    homeMortgagePrincipal: 600000,
    homeMortgageMonthlyPayment: 3500,
    baseMonthlyLivingCost: 2200,
    ...
    marriageDecayPerQuarter: 20,
    marriageMaintainCost: { health: 15, savings: 1200 },
    marriageMaintainGain: 28,
    parentalNagHealthCost: 10
};
...
const game = {
    ...
    init() {
        ...
        this.state = {
            ...
            family: {
                ...
                marriageUnlocked: false,
                marriageChoiceMade: false,
                choseSingle: false,
                isMarried: false,
                marriageHarmony: 0,
                maintainedThisQuarter: false,
                divorced: false
            },
    ...
    normalizeLoadedState(rawState) {
        ...
        if (typeof state.family.hasHome !== 'boolean') state.family.hasHome = false;
        if (typeof state.family.mortgageRemaining !== 'number') state.family.mortgageRemaining = 0;
        if (typeof state.family.mortgageMonthlyPayment !== 'number') state.family.mortgageMonthlyPayment = state.family.hasHome ? GAME_CONFIG.homeMortgageMonthlyPayment : 0;
        if (typeof state.family.marriageUnlocked !== 'boolean') state.family.marriageUnlocked = Boolean(state.family.hasHome || state.family.isMarried || state.family.divorced);
        if (typeof state.family.marriageChoiceMade !== 'boolean') state.family.marriageChoiceMade = Boolean(state.family.isMarried || state.family.divorced);
        if (typeof state.family.choseSingle !== 'boolean') state.family.choseSingle = false;
        if (typeof state.family.isMarried !== 'boolean') state.family.isMarried = false;
        if (typeof state.family.marriageHarmony !== 'number') state.family.marriageHarmony = state.family.isMarried ? 100 : 0;
        if (typeof state.family.maintainedThisQuarter !== 'boolean') state.family.maintainedThisQuarter = false;
        if (typeof state.family.divorced !== 'boolean') state.family.divorced = false;
    ...
    updateStartScreen() {
        ...
        startTip.innerText = '纯前端，直接打开 index.html 即可游玩。';
    },
...
applyQuarterlyPersonalExpenses() {
        const baseCost = GAME_CONFIG.baseMonthlyLivingCost * 3;
        this.changeStat('savings', -baseCost);
        this.log('warning', `🧾 本季度生活支出：${UTILS.formatMoney(baseCost)}（房租、水电、交通和日常开销）`);

        const family = this.state.family;
        if (family && family.hasHome && family.mortgageRemaining > 0) {
            const quarterlyMortgage = Math.min(family.mortgageRemaining, family.mortgageMonthlyPayment * 3);
            family.mortgageRemaining = Math.max(0, family.mortgageRemaining - quarterlyMortgage);
            this.changeStat('savings', -quarterlyMortgage);
            this.log('warning', `🏦 本季度房贷扣款：${UTILS.formatMoney(quarterlyMortgage)}（每月 ${UTILS.formatMoney(family.mortgageMonthlyPayment)}）`);
            if (family.mortgageRemaining === 0) {
                family.mortgageMonthlyPayment = 0;
                this.log('success', '🎉 房贷已全部还清，你终于真正拥有了这套房。');
            }
        }
    },

    settleMarriageQuarter() {
        const family = this.state.family;
        if (!family || !family.isMarried) return;

        family.marriageHarmony = UTILS.clamp(family.marriageHarmony - GAME_CONFIG.marriageDecayPerQuarter, 0, 100);
        if (!family.maintainedThisQuarter) {
            this.log('warning', `💔 本季度没有经营婚姻，亲密度 -${GAME_CONFIG.marriageDecayPerQuarter}。`);
        } else {
            this.log('info', `💞 婚姻进入日常磨合，亲密度自然回落 ${GAME_CONFIG.marriageDecayPerQuarter}。`);
        }

        if (family.marriageHarmony <= 0) {
            this.triggerDivorce();
        }
    },

    applyParentalNagAtQuarterStart() {
        const family = this.state.family;
        if (!family || !family.marriageUnlocked || !family.choseSingle || family.isMarried || family.divorced) return;

        this.changeStat('health', -GAME_CONFIG.parentalNagHealthCost);
        this.showModal(
            '📞 家长催婚',
            `新季度刚开始，家长的电话就打来了：
“你什么时候考虑个人问题？”

你努力解释自己现在不想结婚，但还是被消耗了不少精力。

【后果】
精力 -${GAME_CONFIG.parentalNagHealthCost}`,
            [{ txt: '挂断电话', cb: () => this.closeModal() }]
        );
        this.log('warning', `📞 因为选择不想结婚，本季度开始时被家长催婚，精力 -${GAME_CONFIG.parentalNagHealthCost}。`);
    },

    triggerDivorce() {
        const family = this.state.family;
        const savingsBefore = this.state.player.savings;
        const mortgageBefore = family.mortgageRemaining;
        this.state.player.savings = Math.floor(savingsBefore / 2);
        family.mortgageRemaining = Math.floor(mortgageBefore / 2);
        family.mortgageMonthlyPayment = family.mortgageRemaining > 0 ? Math.max(1, Math.floor(family.mortgageMonthlyPayment / 2)) : 0;
        family.isMarried = false;
        family.marriageHarmony = 0;
        family.maintainedThisQuarter = false;
...
财产分割后：
个人存款由 ${UTILS.formatMoney(savingsBefore)} 变为 ${UTILS.formatMoney(this.state.player.savings)}
剩余房贷由 ${UTILS.formatMoney(mortgageBefore)} 变为 ${UTILS.formatMoney(family.mortgageRemaining)}。`,
            [{ txt: '沉默接受', cb: () => this.closeModal() }]
        );
        this.log('danger', `💔 你离婚了，个人存款被分割一半，剩余房贷也减半。`);
    },
...
nextQuarter() {
        ...
        const proceedEndQuarter = () => {
            ...
            this.applyQuarterlyPersonalExpenses();
            this.settleMarriageQuarter();
            ...
            if (this.state.family) this.state.family.maintainedThisQuarter = false;
            this.applyParentalNagAtQuarterStart();
    ...
    actionShop(type) {
        ...
        if (type === 'coffee') {
            // [修改] 检查存款 savings
            if (this.state.player.savings < 120) { 
            ...
            // [修改] 扣除存款 savings
            this.changeStat('savings', -120);
            this.showResult("精品埃塞豆手磨咖啡佐无糖巴克斯芝士蛋糕", {health: hAdd, mood: mAdd});
            this.log("system", "☕ 花120元私房钱优雅了一下，心情变好了。");
        } else {
            // [修改] 检查存款 savings
            if (this.state.player.savings < 260) { 
            ...
            // [修改] 扣除存款 savings
            this.changeStat('savings', -260);
            this.showResult("享用了博物馆联名正餐套餐", {health: hAdd, mood: mAdd});
            this.log("system", "🍱 花260元私房钱吃了顿好的，充满力量！");
        }
    },
    ...
    updateUI() {
        ...
        this.renderMarriagePanel();
        this.renderUniversityUI();
        this.persistProgress();
    },
    renderHomePanel() {
        const family = this.state.family;
        const canBuyHome = !family.hasHome && this.state.player.savings >= GAME_CONFIG.homeUnlockSavings;
        const unlocked = family.hasHome || canBuyHome;
        ...
        if (!unlocked) {
            placeholder.innerHTML = `
                ...
                <h3>温馨小窝</h3>
                <p>这里是你心灵的港湾，也是人生下一阶段的入口。</p>
                <p style="color:var(--danger); margin-top:10px">购房首付：个人存款达到 ${UTILS.formatMoney(GAME_CONFIG.homeUnlockSavings)}</p>
            `;
            return;
        }
        ...
        if (!family.hasHome) {
            homeContent.innerHTML = `
                <div class="home-panel">
                    <div class="home-panel-title">准备买房</div>
                    <div class="home-panel-copy">你已经攒够首付。支付 ${UTILS.formatMoney(GAME_CONFIG.homeDownPayment)} 后即可买房，并背上每月 ${UTILS.formatMoney(GAME_CONFIG.homeMortgageMonthlyPayment)} 的房贷。</div>
                    <div class="home-rest-summary">购房后：剩余房贷 ${UTILS.formatMoney(GAME_CONFIG.homeMortgagePrincipal)}，并解锁后面的“婚姻系统”。</div>
                    <button class="primary" onclick="game.actionBuyHome()">支付首付并买房</button>
                </div>
            `;
            return;
        }
        ...
        homeContent.innerHTML = `
            <div class="home-panel">
                ...
                <div class="home-panel-copy">房子终于有了，但成年人的生活从来不是通关奖励，而是新的长期支出。</div>
                <div class="home-finance-grid">
                    <div><span>剩余房贷</span><strong>${UTILS.formatMoney(family.mortgageRemaining)}</strong></div>
                    <div><span>月供</span><strong>${UTILS.formatMoney(family.mortgageMonthlyPayment)}</strong></div>
                </div>
    ...
    renderMarriagePanel() {
        const family = this.state.family;
        const marriageTab = document.getElementById('tab-marriage');
        const placeholder = document.querySelector('#view-marriage .scene-placeholder');
        const marriageContent = document.getElementById('marriage-content');
        const unlocked = Boolean(family && family.marriageUnlocked);
        if (marriageTab) marriageTab.classList.toggle('locked', !unlocked);
        if (!placeholder || !marriageContent) return;

        placeholder.classList.toggle('hidden', unlocked);
        marriageContent.classList.toggle('hidden', !unlocked);

        if (!unlocked) {
            placeholder.innerHTML = `
                <div class="scene-icon">🔒</div>
                <h3>婚姻系统</h3>
                <p>先买房，才会开启人生中的情感选择。</p>
                <p style="color:var(--danger); margin-top:10px">解锁条件：在家庭系统中成功买房</p>
            `;
            return;
        }

        const instructionCard = `
            <div class="marriage-guide-card">
                <div class="home-subtitle">📖 婚姻系统游玩说明</div>
                <div class="home-small-copy">
                    1. 买房后，婚姻系统解锁，但不会强迫你立刻结婚。<br>
                    2. 选择“遇到真爱，结婚”后，会出现亲密度；每季度亲密度自然下降，需要消耗精力和存款去经营。<br>
                    3. 亲密度降到 0，会触发离婚：个人存款减半，剩余房贷也减半。<br>
                    4. 选择“不想结婚”后，你可以保持单身，但从之后开始，每个季度一开始都会被家长催婚，固定消耗精力 ${GAME_CONFIG.parentalNagHealthCost} 点。
                </div>
            </div>
        `;

        if (!family.marriageChoiceMade) {
            marriageContent.innerHTML = `
                <div class="marriage-panel">
                    <div class="home-panel-title">婚姻系统已解锁</div>
                    <div class="home-panel-copy">有了房子之后，生活突然多出了一道新的选择题。你可以进入婚姻，也可以选择暂时一个人生活。</div>
                    ${instructionCard}
                    <div class="marriage-choice-grid">
                        <button class="primary" onclick="game.actionChooseMarriage('true-love')">💞 遇到真爱，结婚</button>
                        <button onclick="game.actionChooseMarriage('single')">🧍 不想结婚</button>
                    </div>
                </div>
            `;
            return;
        ...
        if (family.isMarried) {
            const harmony = family.marriageHarmony;
            const maintained = family.maintainedThisQuarter;
            marriageContent.innerHTML = `
                <div class="marriage-panel">
                    <div class="home-panel-title">婚姻生活</div>
                    ${instructionCard}
                    <div class="home-marriage-card">
                        <div class="home-subtitle">💍 婚姻状态</div>
                        <div>亲密度：<strong>${harmony}/100</strong></div>
                        <div class="marriage-bar"><span style="width:${harmony}%"></span></div>
                        <div class="home-small-copy">每季度自然下降 ${GAME_CONFIG.marriageDecayPerQuarter} 点；低至 0 将触发离婚和财产分割。</div>
                        <button onclick="game.actionMaintainMarriage()" ${maintained ? 'disabled' : ''}>${maintained ? '本季度已维护关系' : `经营婚姻（精力 -${GAME_CONFIG.marriageMaintainCost.health}，存款 -${UTILS.formatMoney(GAME_CONFIG.marriageMaintainCost.savings)}）`}</button>
                    </div>
                </div>
            `;
            return;
        ...
        if (family.divorced) {
            marriageContent.innerHTML = `
                <div class="marriage-panel">
                    <div class="home-panel-title">婚姻已结束</div>
                    ${instructionCard}
                    <div class="home-marriage-card divorced">
                        <div class="home-subtitle">💔 当前状态</div>
                        <div>你已经离婚，往后的日子需要独自面对。</div>
                    </div>
                </div>
            `;
            return;
        ...
        marriageContent.innerHTML = `
            <div class="marriage-panel">
                <div class="home-panel-title">单身生活</div>
                ${instructionCard}
                <div class="home-marriage-card single">
                    <div class="home-subtitle">🧍 当前状态</div>
                    <div>你选择了暂时不结婚。</div>
                    <div class="home-small-copy">从下个季度开始，每季度开头都会被家长催婚一次，精力 -${GAME_CONFIG.parentalNagHealthCost}。</div>
                    <button class="primary" onclick="game.actionChooseMarriage('true-love')">后来遇到真爱，选择结婚</button>
                </div>
            </div>
        `;
    },
    ...
    switchScene(sceneName) {
        ...
        if (sceneName === 'home') {
            const family = this.state.family;
            const canEnterHome = family.hasHome || this.state.player.savings >= GAME_CONFIG.homeUnlockSavings;
            if (!canEnterHome) {
                this.showResult("未解锁", `买房首付还没攒够呢！(需要存款≥${UTILS.formatMoney(GAME_CONFIG.homeUnlockSavings)})`);
                return;
            }
        }
        if (sceneName === 'marriage') {
            const family = this.state.family;
            if (!family || !family.marriageUnlocked) {
                this.showResult("未解锁", "先在家庭系统中买房，婚姻系统才会开启。");
                return;
            }
        }
        // 2. 切换 UI 显示
    ...
    actionStudy(type) {
        ...
        if (type === 'course') {
            if (this.state.flags.hasStudiedThisQuarter) {
                this.showResult("进修受限", "本季度只能进修课程一次，难道你不累吗。");
                return;
            }

            if (this.state.player.savings < GAME_CONFIG.studyCourseCost) {
                this.showResult("存款不足", "学费不够，还是先去打工吧。");
                return;
            }
            ...
            const courses = [
                {
                    title: "博物馆管理与运营",
                    feedback: "你进修了清北大学的博物馆管理与运营课程，受益匪浅。"
                },
                {
                    title: "文物保护与修复",
                    feedback: "你进修了帝都美术学院文保学院的文物保护与修复课程，对专业规范有了更深理解。"
                },
                {
                    title: "数智化建模课程",
                    feedback: "你进修了下海设计学院的数智化建模课程，开始理解空间表达的技术逻辑。"
                },
                {
                    title: "数字博物馆建设",
                    feedback: "你进修了西京大学的数字博物馆建设课程，思考工作中更多的可能性。"
                },
                {
                    title: "博物馆教育设计",
                    feedback: "你进修了帝都师范大学的博物馆教育设计课程，对观众体验更加敏感。"
                }
            ];
    ...
    showIntro() {
        ...
        const content = [
            ...
            '<button type="button" id="style-bento" class="intro-style-btn">好好摸鱼</button>',
            '<button type="button" id="style-brutal" class="intro-style-btn active">认真干活</button>',
            '</div>',
            '</div>',
            '</div>'
        ].join('');
    ...
    hireSubstitute(courseId) {
        ...
        if (this.state.player.savings < 2600) {
    ...
    actionBuyHome() {
        const family = this.state.family;
        if (family.hasHome) {
            this.showResult('已经买房', '你已经有房了，先把月供还完吧。');
            return;
        }
        if (this.state.player.savings < GAME_CONFIG.homeDownPayment) {
            this.showResult('首付不足', `需要至少 ${UTILS.formatMoney(GAME_CONFIG.homeDownPayment)} 才能买房。`);
            return;
        }

        this.markAction();
        this.changeStat('savings', -GAME_CONFIG.homeDownPayment);
        family.hasHome = true;
        family.mortgageRemaining = GAME_CONFIG.homeMortgagePrincipal;
        family.mortgageMonthlyPayment = GAME_CONFIG.homeMortgageMonthlyPayment;
        family.marriageUnlocked = true;
        family.marriageChoiceMade = false;
        family.choseSingle = false;
        family.isMarried = false;
        family.marriageHarmony = 0;
        family.maintainedThisQuarter = false;
        ...
        this.showModal(
            '🏠 买房成功，人生进入下一阶段',
            `你支付了 ${UTILS.formatMoney(GAME_CONFIG.homeDownPayment)} 首付，买下了属于自己的房子。

剩余房贷：${UTILS.formatMoney(family.mortgageRemaining)}
每月月供：${UTILS.formatMoney(family.mortgageMonthlyPayment)}

婚姻系统已解锁，请到“婚姻”界面选择你接下来的人生道路。`,
            [{ txt: '去看看婚姻系统', cb: () => { this.closeModal(); this.switchScene('marriage'); } }]
        );
        this.log('success', `🏠 你支付首付买了房，剩余房贷 ${UTILS.formatMoney(family.mortgageRemaining)}，婚姻系统已解锁。`);
        this.updateUI();
    },

    actionChooseMarriage(choice) {
        const family = this.state.family;
        if (!family.marriageUnlocked) {
            this.showResult('尚未解锁', '请先在家庭系统中买房。');
            return;
        }
        if (family.divorced) {
            this.showResult('无法重新选择', '这段婚姻已经结束，当前版本不再重新开启婚姻选择。');
            return;
        }

        this.markAction();
        family.marriageChoiceMade = true;
        if (choice === 'true-love') {
            family.choseSingle = false;
            family.isMarried = true;
            family.marriageHarmony = 100;
            family.maintainedThisQuarter = false;
            this.showResult('你遇到了真爱，决定携手进入婚姻。', {});
            this.log('success', '💞 你遇到了真爱，选择结婚，婚姻亲密度从 100 开始。');
        } else {
            family.choseSingle = true;
            family.isMarried = false;
            family.marriageHarmony = 0;
            family.maintainedThisQuarter = false;
            this.showResult('你决定暂时不结婚，先把人生过成自己的样子。', {});
            this.log('info', `🧍 你选择暂时不结婚；从下季度开始，每季度开头会被家长催婚，精力 -${GAME_CONFIG.parentalNagHealthCost}。`);
        }
        this.updateUI();
    },

    actionMaintainMarriage() {
        const family = this.state.family;
        if (!family.hasHome || !family.isMarried) {
            this.showResult('无法经营', '当前没有需要维护的婚姻关系。');
            return;
        }
        if (family.maintainedThisQuarter) {
            this.showResult('已经陪伴过了', '本季度已经认真经营过婚姻，下季度再继续。');
            return;
        }
        if (this.state.player.health <= GAME_CONFIG.marriageMaintainCost.health) {
            this.showResult('精力不足', '你已经累得说不出一句完整的话，先休息再谈感情吧。');
            return;
        }
        if (this.state.player.savings < GAME_CONFIG.marriageMaintainCost.savings) {
            this.showResult('存款不足', `经营婚姻也需要现实成本，至少需要 ${UTILS.formatMoney(GAME_CONFIG.marriageMaintainCost.savings)}。`);
            return;
        }

        this.markAction();
        this.changeStat('health', -GAME_CONFIG.marriageMaintainCost.health);
        this.changeStat('savings', -GAME_CONFIG.marriageMaintainCost.savings);
        family.marriageHarmony = UTILS.clamp(family.marriageHarmony + GAME_CONFIG.marriageMaintainGain, 0, 100);
        family.maintainedThisQuarter = true;
        this.showResult('你认真安排了一次相处，让关系重新回暖。', {
            health: -GAME_CONFIG.marriageMaintainCost.health,
            savings: -GAME_CONFIG.marriageMaintainCost.savings
        });
        this.log('info', `💞 你花时间经营婚姻，亲密度恢复到 ${family.marriageHarmony}/100。`);
        this.updateUI();
    },

    actionHomeRest() {
        if (!this.state.family.hasHome) {
            this.showResult("未解锁", `需要先支付首付买房，才能真正拥有一个家。`);
            return;
        }
...
showGuide() {
        this.guideSteps = [
            ...
            },
            {
                ...
                scene: "office", 
                text: `🏠 家庭系统
当你的存款达到 ${UTILS.formatMoney(GAME_CONFIG.homeUnlockSavings)} 时，可以支付首付买房。
买房后每个季度都要偿还房贷，同时可以回家休息一次，恢复精力和愉悦。
成功买房后，还会解锁后面的【婚姻系统】。`
            },
            {
                selector: "#tab-marriage",
                scene: "office",
                text: "💍 婚姻系统\n买房后会解锁【婚姻】界面。\n你可以选择【遇到真爱，结婚】进入亲密度维护玩法；也可以选择【不想结婚】保持单身。\n如果选择不结婚，之后每个季度开始都会被家长【催婚】，并消耗精力。\n若婚后亲密度降到 0，会触发离婚：个人存款减半，剩余房贷减半。"
            },
            {
